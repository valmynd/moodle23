...
class qtype_rtypetask_edit_form extends elate_question_edit_form {
	...
	public function get_answer_definition_html($num_current_subquestion) {
		...
		//$this->_form->setDefaults($_POST);
		//moodleform::set_data($_POST);
		$this->_process_submission('post'); // THIS hanles overriding values from $_POST
		$form_html = $this->_form->toHtml();
		// we only need the things between <section></section> -> (much easier) handled in JS, now!
