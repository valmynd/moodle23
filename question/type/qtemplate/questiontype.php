...
/**
 * Question type class for the qtemplate question type.
 *
 * @package    qtype
...
/**
 * The qtemplate question type class.
 *
 * TODO: Make sure short answer questions chosen by a qtemplate question
 * can not also be used by a random question
 *
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
*/
class qtype_qtemplate extends question_type {
	...
	public function get_question_options($question) {
		global $DB;
		$question->options = $DB->get_record('question_qtemplate',
	...
	public function save_question_options($question) {
		...
		if ($existing = $DB->get_record('question_qtemplate',
				array('question' => $options->question))) {
			$options->id = $existing->id;
			$DB->update_record('question_qtemplate', $options);
		} else {
			$DB->insert_record('question_qtemplate', $options);
		}
		return true;
	}
	...
	public function delete_question($questionid, $contextid) {
		global $DB;
		$DB->delete_records('question_qtemplate', array('question' => $questionid));
